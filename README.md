...
This repository contains automation scripts to create new namespaces with the necessary setup for the IBM Blockchain operational tooling and deploy said tooling into each of these namespaces for a lab setup. You can clean up all artifacts including helm releases, the clusterrole, and the namespaces with all of the other deployed resources.
...
Note: Chrome is not supported at this time. Please use Firefox or Safari but not IBM Firefox (since it uses Firefox ESR [Firefox Extended Support Release] instead of the latest)Additionally, you may need to delete your web browser cache from an earlier optools instance for things to load properly if you are having problems seeing the sign-on screen.
...
**You can find the details for accessing each console in the note of the optools helm chart**
...
It took 4 minutes and 56 seconds to setup 15 optools instances each in an unique namespace
```
...
It took 3 minutes and 32 seconds to cleanup 15 optools instances and their namespaces
```
