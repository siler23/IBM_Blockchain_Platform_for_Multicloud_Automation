...
set -x
helm install --tls --namespace ${NAMESPACE} ${HELM_REPO}/ibm-blockchain-platform-prod --version ${PROD_VERSION} -n ${NAME} \
--set app.email=${EMAIL} \
...
--set ingress.optools.hostname=${PROXY_IP} \
--set ingress.optools.port=${OPTOOLS_PORT} \
--set ingress.proxy.hostname=${PROXY_IP} \
...
if [ $RC != 0 ]; then
    echo "Optools/operator $NAME Deployment Failed"
    exit 1
fi